...
from PyQt6.QtWidgets import QGroupBox, QLabel, QVBoxLayout, QHBoxLayout, QSizePolicy, QSpinBox
...
class SettingsStep(Step):
    def __init__(self, *, text: str, previous_text="Schließen", previous_callback=None, next_text="Speichern",
        ...
        self.crop_box_top_layout, self.crop_box_top_spinner = \
            SettingsStep._create_spinbox(default_value=top, label="Oberer Rand:")

        self.crop_box_right_layout, self.crop_box_right_spinner = \
            SettingsStep._create_spinbox(default_value=right, label="Rechter Rand:")

        self.crop_box_bottom_layout, self.crop_box_bottom_spinner = \
            SettingsStep._create_spinbox(default_value=bottom, label="Unterer Rand:")

        self.crop_box_left_layout, self.crop_box_left_spinner = \
    ...
    def _create_spinbox(*, default_value: int, label: str) -> (QHBoxLayout, QSpinBox):
        ...
        crop_box_label.setSizePolicy(QSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed))
        crop_box_label.setMinimumWidth(80)

        crop_box_spinner = create_mm_spinbox(default_value=default_value)
        crop_box_spinner.setSizePolicy(QSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed))
    ...
    def get_crop_box(self) -> (int, int, int, int):
        return self.crop_box_top_spinner.value(), self.crop_box_right_spinner.value(), \
               self.crop_box_bottom_spinner.value(), self.crop_box_left_spinner.value()
